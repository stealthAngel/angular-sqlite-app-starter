  ...
  async runTest(): Promise<void> {
    try {
      ...
      if (retCC && isConn) {
        ...
        db = await this._sqlite.createConnection("testNew", false, "no-encryption", 1);
      }
      isConn = (await this._sqlite.isConnection("testSet")).result;
      if (retCC && isConn) {
        db1 = await this._sqlite.retrieveConnection("testSet");
      } else {
        db1 = await this._sqlite.createConnection("testSet", true, "secret", 1);
      }
      ...
      let sqlcmd: string = "INSERT INTO users (name,email,age,size,company) VALUES (?,?,?,?,?)";
