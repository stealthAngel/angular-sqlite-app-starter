...
export class DownloadFromHTTP implements OnInit {
  ...
  async runTest(): Promise<void> {
    ...
    try {
      ...
      if (retTables.values.length !== 3 || !retTables.values.includes("areas") || !retTables.values.includes("elements") || !retTables.values.includes("issues")) {
        return Promise.reject("GetTableList dbZip1 Tables failed");
      }
      const resAreas = await db1.query("SELECT * FROM areas");
      if (resAreas.values.length !== 3 || resAreas.values[0].name !== "Access road" || resAreas.values[1].name !== "Accessway" || resAreas.values[2].name !== "Air handling system") {
        throw new Error("Query 3 areas failed");
      }
